...
*1. I need 6 components to explain at least 50% of the variance in the data 
<br>*2. 5 yr T-Notes US (0.93); 10yr T-Notes (0.93);Treasury Bonds (0.86); 2 yr T-Note US (0.85); Canadian Bond (0.79)
<br>*3. The first 10 components explain 62.61% of the variance and can be called principal components: Factor1 Government bonds, Factor 2 Exchange rates, Factor 3 EU stock market indices, Factor 4 North America stock market indices, Factor 5 short term interest rate, Factor 6 oil derivatives, Factor 7 metals (eg. zinc), Factor 8 Agrifood (eg. soybean), Factor 9 precious metals (eg. gold), Factor 10 Asia stock indices 
<br>*4. introduced the below code: <br>crisis_data=futures_data[as.Date(rownames(futures_data))>"2007-10-01"&as.Date(rownames(futures_data))<"2009-03-01", ]
<br>Variance_Explained_Table_results<-PCA(crisis_data, graph=FALSE)
<br>*5. i see that during the crisis i only need 4 components to explain at least 50% of the variance in the data (vs. 6 if i consider the whole perido). Also the pct explained by the 1st component explains approx. more 40% (from 21 to 29)
*
*
*
*
...
*1. First step sets the signal to 0;
Second step defines a frequency over which the frequency should move to create a moving average;
Third step sets the window of time over which this moving average should be calculated;
Fourth step defines that the signal  of the output of the function (positive, negative, neutral) and should be multiplied;
Fifth step weights the sign of each output until is reached the cumulative sum of 1 by scaling the sum across all the futures;
Sixth step investment strategy is applied: we invest by applying the weights we calculated 2 days ago to the futures.
<br>*2. The command line MAfreq<-seq(10,250,by=20), affects how often the trading strategy is implemented, it affects the frequency. If you use a higher frequency, the strategy becomes less sensitive and returns evolution details are reduced presenting much higher jumps.
*
*
*
*
